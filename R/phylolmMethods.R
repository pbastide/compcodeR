...
getTree <- function(cdata) {
  if (is.null(phylo.tree(cdata)) || length(phylo.tree(cdata)) == 0) {
    ...
    if (length(tree$tip.label) != ncol(count.matrix(cdata))) {
      sample.annotations <- sample.annotations(cdata)
      sample.annotations$id <- colnames(count.matrix(cdata))
      sample.annotations$id.species <- tree$tip.label[sample.annotations$id.species]
      tree <- addReplicatesOnTree(tree, sample.annotations, species = "id.species", id = "id")
    }
    return(tree)
  }
}
...
#' @param id Name of the column containing samples ids. Default to "id". If "auto", the identifiers are automatically generated from the species names.
...
#' @keyword internal
#'
addReplicatesOnTree <- function(tree, traits,
  ...
  if (is.null(traits[[id]]) && id == "auto") {
    id <- "id"
    traits[[id]] <- make.unique(traits[[species]], sep = "_")
  }
  ...
  id_species <- paste0(traits[[species]], tempfile(pattern = "tmp", tmpdir = ""))
  traits[[species]] <- id_species
  tree$tip.label <- id_species[tree_data_cor]
  ## Make tree
  tree_rep <- tree
  # Add replicates
...
phylolm_analysis <- function (dat, design_data, design_formula, tree, model, measurement_error, 
                              upper.bound, lower.bound, starting.value,
                              ...) 
{
  data_reg <- design_data
  data_reg$expr <- dat
  levels(data_reg$condition) <- c(1, 2)
  res <- try(extract_results_phylolm(suppressWarnings(phylolm::phylolm(paste('expr', paste(as.character(design_formula), collapse = '')),
                                                                       data = data_reg,
                                                                       phy = tree,
                                                                       model = model,
